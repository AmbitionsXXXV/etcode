...
export namespace Agent {
	...
	function builtin(): Record<string, Info> {
		return {
			...
			explore: {
				name: 'explore',
				description:
					'Fast agent specialized for exploring codebases with read-only access.',
				mode: 'subagent',
				permission: Permission.merge(
					defaults,
					Permission.fromConfig({
	...
	async function state(): Promise<Record<string, Info>> {
		...
		try {
			...
			for (const [key, value] of Object.entries(cfg.agent ?? {})) {
				...
				if (value.model)
					item.model = { providerID: value.model, modelID: value.model }
	...
	export async function defaultAgent(): Promise<string> {
		...
		const primary = Object.values(agents).find(
