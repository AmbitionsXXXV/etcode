...
async function resolveModel(agent: Agent.Info): Promise<Provider.Model> {
	if (agent.model) {
		const found = await Provider.getModel(
			agent.model.providerID,
			agent.model.modelID
		)
		if (found) return found
		const resolved = Provider.resolveModel(agent)
		if (!resolved)
			...
			execute: (args: unknown, options) => {
				const ctx = {
					...
					metadata(_input: { title?: string; metadata?: Record<string, unknown> }) {
						/* noop — metadata handled by processor */
					},
					async ask(askInput: {
						permission: string
						patterns: string[]
						...
						await Permission.ask({
							sessionID: input.sessionID,
							permission: askInput.permission,
							patterns: askInput.patterns,
							metadata: askInput.metadata,
							always: askInput.always,
							ruleset: input.agent.permission,
							tool: { messageID: input.messageID, callID: options.toolCallId },
						})
					},
				}
	...
	export async function loop(input: {
		...
		try {
			...
			if (input.userMessageID) {
				...
				}).catch((e) => log.warn('summarize failed', { error: String(e) }))
			}
		}
