    ...
    if (removeText && content.includes(removeText)) {
      ...
      if (idx !== -1) {
        ...
        if (removeText) {
          ...
          if (removeIdx !== -1)
            content = content.slice(0, removeIdx) + addText + content.slice(removeIdx + removeText.length)
        } else if (addText) {
...
export const ApplyPatchTool = Tool.define("apply_patch", {
  ...
  async execute(params, ctx) {
    ...
    for (const hunk of hunks) {
      ...
      switch (hunk.type) {
        case "add": {
          ...
          const diff = trimDiff(createTwoFilesPatch(filePath, filePath, "", newContent))
          let additions = 0
          for (const change of diffLines("", newContent))
            if (change.added) additions += change.count || 0
          fileChanges.push({ filePath, oldContent: "", newContent, type: "add", diff, additions, deletions: 0 })
          totalDiff += diff + "\n"
          break
        }
        case "update": {
          ...
          fileChanges.push({
            ...
            movePath, diff, additions, deletions,
          })
          totalDiff += diff + "\n"
          break
        }
        case "delete": {
          ...
          fileChanges.push({
            filePath, oldContent: contentToDelete, newContent: "", type: "delete",
            diff: deleteDiff, additions: 0, deletions: contentToDelete.split("\n").length,
          })
    ...
    const relativePaths = fileChanges.map((c) => path.relative(Instance.directory(), c.filePath))
    await ctx.ask({
      permission: "edit",
    ...
    const summaryLines = fileChanges.map((change) => {
      if (change.type === "add") return `A ${path.relative(Instance.directory(), change.filePath)}`
      if (change.type === "delete") return `D ${path.relative(Instance.directory(), change.filePath)}`
