		...
		if (linesToCheck > 0) {
			for (let j = 1; j < searchBlockSize - 1 && j < actualBlockSize - 1; j++) {
				const maxLen = Math.max(
					originalLines[startLine + j].trim().length,
					searchLines[j].trim().length
				)
				if (maxLen === 0) continue
				similarity +=
					(1 -
						levenshtein(originalLines[startLine + j].trim(), searchLines[j].trim()) /
							maxLen) /
					linesToCheck
	...
	for (const line of lines) {
		if (normalizeWhitespace(line) === normalizedFind) {
			...
			if (normalizedLine.includes(normalizedFind)) {
				const words = find.trim().split(WHITESPACE_SPLIT_REGEX)
				if (words.length > 0) {
					const pattern = words
