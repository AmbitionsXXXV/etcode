...
export const EditTool = Tool.define("edit", {
  ...
  async execute(params, ctx) {
    ...
    if (params.oldString === "") {
      ...
      return {
        metadata: { diff, filediff: undefined as any },
        title: path.relative(Instance.directory(), filePath),
        output: existed ? "Edit applied successfully." : "File created successfully.",
      }
    }
    ...
    const filediff = { file: filePath, before: contentOld, after: contentNew, additions: 0, deletions: 0 }
  ...
  if (candidates.length === 1) {
    ...
    if (linesToCheck > 0) {
      for (let j = 1; j < searchBlockSize - 1 && j < actualBlockSize - 1; j++) {
        const maxLen = Math.max(originalLines[startLine + j].trim().length, searchLines[j].trim().length)
        if (maxLen === 0) continue
        similarity += (1 - levenshtein(originalLines[startLine + j].trim(), searchLines[j].trim()) / maxLen) / linesToCheck
  ...
  for (const candidate of candidates) {
    ...
    if (linesToCheck > 0) {
      for (let j = 1; j < searchBlockSize - 1 && j < actualBlockSize - 1; j++) {
        const maxLen = Math.max(originalLines[startLine + j].trim().length, searchLines[j].trim().length)
        if (maxLen === 0) continue
        similarity += 1 - levenshtein(originalLines[startLine + j].trim(), searchLines[j].trim()) / maxLen
      }
