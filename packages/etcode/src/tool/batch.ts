      ...
      const executeCall = async (call: (typeof toolCalls)[0]) => {
        ...
        try {
          if (DISALLOWED.has(call.tool))
            throw new Error(`Tool '${call.tool}' is not allowed in batch. Disallowed tools: ${Array.from(DISALLOWED).join(", ")}`)

          const tool = toolMap.get(call.tool)
          if (!tool) {
            const available = Array.from(toolMap.keys()).filter((name) => name !== "invalid" && name !== "batch")
            throw new Error(`Tool '${call.tool}' not in registry. Available tools: ${available.join(", ")}`)
          }
