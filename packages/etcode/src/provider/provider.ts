...
export namespace Provider {
	...
	const BUNDLED_PROVIDERS: Record<
		string,
		(options: Record<string, unknown>) => SDK
	> = {
		'@ai-sdk/openai-compatible': (opts) =>
			createOpenAICompatible(opts as any) as unknown as SDK,
		...
		'@ai-sdk/google': (opts) =>
			createGoogleGenerativeAI(opts as any) as unknown as SDK,
	}
	...
	export async function getSDK(model: Model): Promise<SDK> {
		...
		if (!factory) {
			throw new Error(
				`unsupported SDK package: ${model.api.npm}. Bundled: ${Object.keys(BUNDLED_PROVIDERS).join(', ')}`
			)
		}

		log.info('creating SDK instance', {
	...
	export async function getLanguage(model: Model): Promise<LanguageModel> {
		...
		log.info('created language model', {
			providerID: model.providerID,
			modelID: model.id,
		})
		return language
	...
	export function list(): Promise<Record<string, Info>> {
		return state()
	}
